...
\textbf{Problema}  Nella fase di gioco la parte implementativa legata al Model è piuttosto corposa, di conseguenza la logica per ogni update chiamato dal Controller potrebbe creare delle classi troppo grandi con molti compiti diversi.

\textbf{Soluzione}  Suddivisione della parte del Controller legata alla fase di gioco in più classi, seguendo l' \textit{Observer pattern}.
...
\textbf{Problema} Bisogna gestire ogni tipo di collisione, in maniera comprensibile e facile da riutilizzare in diverse situazioni.

\textbf{Soluzione} Creazione della classe Collider che gestisce al suo interno tutta la logica che riguarda il controllo delle possibili collisioni, rendendo l'utilizzo dei suoi metodi veloce ed intuitivo. La classe sfrutta il pattern \textit{Facade}, in quanto fornisce alle altre classi un'interfaccia semplice tramite i propri metodi, nascondendo la logica implementativa. La realizzazione è riassunta in Figura~\ref{fig:uml_collider}.
\begin{figure}[H]
    \centering
...
\textbf{Problema} Serve una funzionalità in grado di manipolare in modo semplice e intuitivo i vari file audio presenti nel gioco.

\textbf{Soluzione} Sviluppo della utility class AudioManager, che anch'essa sfrutta il pattern \textit{Facade} per offrire una interfacciabilità comoda tramite i propri metodi statici. La classe si serve della libreria \textit{javax.sound.sampled} per implementare pochi ma efficienti metodi per il progetto. La realizzazione è riassunta in Figura~\ref{fig:uml_audiomanager}.
\begin{figure}[H]
    \centering
...
\textbf{Problema} Necessità di creare la classe che gestisce il giocatore, rendendola semplice e possibilmente modificabile per eventuali cambiamenti futuri.

\textbf{Soluzione} Sfruttare il pattern \textit{Strategy} per creare una gerarchia
...
\begin{itemize}
  \item \textbf{Enemy}: nel quale viene testato il movimento dei nemici in modo correto, la correta crazione dei tipi di nemici nel livello.
  ...
  \item \textbf{Map}: nel quale viene testata il correto caricamento delle mappe dei livelli 1 e 2, 
...
\end{sloppypar}
\paragraph{Uso di librerie di terza parte(SpotBugs / FindBugs)}
\begin{sloppypar}
  \url{https://github.com/burraco04/OOP25-hop-tales/blob/e8d6cd9bf68d214d4bf5f25293e49066bc76d247/src/main/java/model/entities/impl/AbstractEnemyImpl.java#L3}
\end{sloppypar}
\begin{sloppypar}
  \url{https://github.com/burraco04/OOP25-hop-tales/blob/e8d6cd9bf68d214d4bf5f25293e49066bc76d247/src/main/java/controller/impl/EnemyController.java#L4}
\end{sloppypar}
...
una volta entrati in start e selezionato un livello si potra muovere il player premendo \textbf{A} per muoverlo a sinistra, \textbf{D} per muoverlo a destra, 
\textbf{W} oppure \textbf{barra spaziatrice} per saltare (se tenuto premuto continua a saltare ogni volta che tocca un blocco solido) e \textbf{S} per cadere più velocemente (Solo quando si è in aria).
Una volta raggiunta la porta a fine livello 1 e 2 si potrsa preme \textbf{E} per tornale al menu principale.
Invece nel livello 3 si potrà muovere due player uno utilizzando \textbf{A} per muoverlo a sinistra, \textbf{D} per muoverlo a destra, 
\textbf{W} per saltare e l'altro player userà \textbf{freccia →} per muoverlo a sinistra, \textbf{freccia ←} per muoverlo a destra, 
\textbf{freccia ↑} per saltare.
